...
"""
    ...
    for cat in categories:
        ...
        print("CATEGORY:", cat)
        print("SUB_CATEGORY UNIQUE:", df_cat["sub_category"].unique())
# DEBUG start
    # for cat in categories:
        # df_cat = df[df["category"] == cat]
        
        # 🔍 DEBUG: بررسی داده‌های هر category
        # print(f"\n{'='*60}")
        ...
        # for _, row in df_cat.iterrows():
            # print(f"\nTest: {row['test_name']}")
            # print(f"  is_numeric: {row.get('is_numeric', 'NOT FOUND')}")
            # print(f"  value: {row['value']} (type: {type(row['value'])})")
            # print(f"  unit: {row['unit']}")
            # print(f"  ref_low: {row.get('ref_low')}")
            # print(f"  ref_high: {row.get('ref_high')}")
            # print(f"  reference_full_text: {str(row.get('reference_full_text', 'EMPTY'))[:50]}...") 
...
# DEBUG end
        ...
        def highlight_row(row):
            ...
            result = row["Result"] if "Result" in row.index else ""
        ...
        if has_sub:
            for sub in sub_cats:
                ...
                # ==========================================
                # ستون سمت چپ: رسم جدول + رسم Pie Chart
                # ==========================================
                with col_left:
                    # الف) رسم جدول
                    sub_display_html = sub_display.copy()
                    sub_display_html["Reference Range"] = sub_display_html["Reference Range"].fillna("").astype(str).str.replace('\n', '<br>')
                ...
                # ==========================================
                # ستون سمت راست: رسم Bullet Chartها
                # ==========================================
                with col_right:
                    for _, row in df_sub.iterrows():
                        row_is_numeric = row.get("is_numeric", 1)

                        # رد کردن تست‌هایی که نباید بولت چارت داشته باشند (0 و 3)
    ...
    # بقیه کد تو که مربوط به branch بدون sub_category هست از اینجا شروع میشه...
            col_left, col_right = st.columns([1, 1.2])
            # ---------- LEFT ----------
            with col_left:
                # HTML Table
                display_df_html = display_df.copy()
                display_df_html["Reference Range"] = display_df_html["Reference Range"].fillna("").astype(str).str.replace('\n', '<br>')
                html_table = display_df_html.to_html(index=False, escape=False, classes="lab-table")
                st.markdown(css_style + html_table, unsafe_allow_html=True)
            ...
            # ✅ Pie Chart فقط برای is_numeric = 3
            ...
            with col_right:
                for _, row in df_cat.iterrows():
                    ...
                    if row_is_numeric == 3:
                        continue
                    if "Microalbumin /Creat Random" in str(row["test_name"]):
                        print("\n--- CONDITION CHECK ---")
                        print("Test:", row["test_name"])
                        print("is_numeric_val:", pd.notnull(row["value"]))
                        print("has_ref_text:", pd.notnull(row.get("reference_full_text")) and str(row.get("reference_full_text","")).strip()!="")
                        print("has_ref_range:", pd.notnull(row.get("ref_low")) and pd.notnull(row.get("ref_high")))
                        print("-----------------------\n")
                    has_ref_text = pd.notnull(row.get("reference_full_text")) and str(row.get("reference_full_text", "")).strip() != ""
                    has_ref_range = pd.notnull(row.get("ref_low")) and pd.notnull(row.get("ref_high"))
                    is_numeric_val = pd.notnull(row["value"])

                    if is_numeric_val and (has_ref_text or has_ref_range):
                        if "Microalbumin /Creat Random" in str(row["test_name"]):
                            print("\n====== DEBUG MICROALBUMIN ======")
                            print("Test:", row["test_name"])
                            print("Value:", row["value"], type(row["value"]))
                            print("Reference text:\n", row["reference_full_text"])
                            parsed = parse_multi_range(row["reference_full_text"])
                            print("Parsed ranges:", parsed)
                            print("================================\n")
                        fig = create_clinical_range_chart(row, pdf_mode=False)
                        if fig:
                            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
                        else:
                            # اگر نمودار نشد، متن نمایش بده
                            status_color = "#FF8C00" if row["status"] == "Low" else ("red" if row["status"] == "High" else "green")
                            st.markdown(f'<div style="border-left: 5px solid {status_color}; padding: 10px; margin: 5px; background: #f9f9f9;"><span style="font-weight: bold; font-size: 16px;">{row["test_name"]}:</span> <span style="font-size: 18px; margin-left: 20px;">{row["value_clean"]} {row.get("unit", "")}</span><span style="float: right; color: {status_color}; font-weight: bold;">{row["status"]}</span></div>', unsafe_allow_html=True)
                    else:
                        status_color = "#FF8C00" if row["status"] == "Low" else ("red" if row["status"] == "High" else "green")
