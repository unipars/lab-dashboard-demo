...
pio.defaults.kaleido_timeout = 30000
...
"""
    ...
    with st.sidebar:
        ...
        summary_data = [
            ...
            [
                " Normal",
                str(len(normal_tests)),
                Paragraph("", wrap_style)
            ],
                        [
                " High",
                str(len(high_tests)),
                Paragraph(", ".join(high_tests[:10]) + ("..." if len(high_tests) > 10 else ""), wrap_style)
            ],
            [
                " Low",
                str(len(low_tests)),
                Paragraph(", ".join(low_tests[:10]) + ("..." if len(low_tests) > 10 else ""), wrap_style)
            ],
        ]
        ...
        for cat in categories:
            ...
            if has_sub:
                        ...
                        if is_numeric_val and (has_ref_text or has_ref_range):
                            ...
                            if fig:
                                try:
                                    ...
                                    print("Bullet PDF export error:", e)
                ...
                # ✅ Bullet Charts برای دسته بدون sub_category
                for _, row in df_cat.iterrows():
                    ...
                    has_ref_text = (
                        pd.notnull(row.get("reference_full_text")) and
                        str(row.get("reference_full_text", "")).strip() != ""
                    )
                    ...
                    is_numeric_val = (
                        ...
                        pd.api.types.is_number(row.get("value"))
                    )

                    if is_numeric_val and (has_ref_text or has_ref_range):
                        fig = create_clinical_range_chart(
                            row,
                            pdf_mode=True,
                            width=AVAILABLE_WIDTH * 1.5
                        )
                        ...
                        if fig:
                            try:
                                ...
                                print("PDF bullet export error:", e)
